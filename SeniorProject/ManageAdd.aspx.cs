...
public partial class Manage : System.Web.UI.Page
{
     ...
     * 
     */
    private const string DROPLIST_COUNTRY_TEXT = "user...";
    private const string DROPLIST_TOPIC_TEXT = "topic...";
    private const string DROPLIST_GRADE_TEXT = "environment...";
    private const string DROPLIST_COUNTRY_GRADE_TEXT = "user environment...";
    private const string DROPLIST_COUNTRY_GRADE_TOPIC_TEXT = "user environment topic...";
    ...
    private void UpdateAllData()
    {
        ...
        if (!Updateuser(connection))
            return;

        if (!Updateenvironment(connection))
            return;
    ...
    /*
     * 
     * Method for updating data related to 'user'. Reverting 'country'
     * droplist back to original state and re-populating it with the current
     * 'country data' from the database.
     * 
     */
    private bool Updateuser(MySqlConnection connection)
    {
    ...
    /*
     * 
     * Loads in the new environment everytime a page load is called
     * 
     */
    private bool Updateenvironment(MySqlConnection connection)
    {
    ...
    private bool UpdateCountryGrade(MySqlConnection connection)
    {
        ...
        try
        {
            ...
            //display the user
            String sql = "SELECT userType,env FROM user_env_relationship ORDER BY userType,env ASC";
    ...
    private bool UpdateCountryGradeTopic(MySqlConnection connection)
    {
        ...
        try
        {
            ...
            //display the user
            String sql = "SELECT userType,env,tid FROM user_env_topic_relation ORDER BY userType,env,tid ASC";
    ...
    private bool UpdateLessons(MySqlConnection connection)
    {
        ...
        try
        {
            ...
            String sql = "SELECT userType,env,tid,lid,text,path,filename FROM lessons ORDER BY userType,env,tid,lid ASC";
     ...
     * Then attempts to check that there are environment that can still be associated
     * with that given country.
     * If okay, then the environment will populate the grade droplist
     * If failure occurs, log messages will be produced and
    ...
    protected void CountryGrade_country_IndexChange(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //this selects all the env from the 'environment' tables that does not yet
            //have an association with the given country
            String sql = "SELECT env " +
                         "FROM environment " +
                         "WHERE env NOT IN " +
                            "(SELECT env FROM user_env_relationship WHERE userType = (@userType));";
            ...
            using (MySqlCommand cmd = new MySqlCommand(sql, connection))
            {
                ...
                using (MySqlDataReader rdr = cmd.ExecuteReader())
                {
                    if (!rdr.HasRows)
                    {
                        tblog.Text += Environment.NewLine + "~Country_Grade: all environment are associated with " + country + ", no more environment to add";
                        canContinue = false;
                    }
    ...
    protected void AddCountryGrade_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            String sql = "INSERT INTO user_env_relationship (userType,env) VALUES (@userType,@env)";
    ...
    protected void CountryGradeTopic_countrygrade_IndexChange(object sender, EventArgs e)
    {
        ...
        string[] countryenvironmentplit = tempCountryGrade.Split('|');
        string country = countryenvironmentplit[0];
        string grade = countryenvironmentplit[1];
        ...
        try
        {
            ...
            String sql = "SELECT tid " +
                         "FROM topics " +
                         "WHERE tid NOT IN " +
                            "(SELECT tid FROM user_env_topic_relation WHERE userType = (@userType) AND env = (@env));";
    ...
    protected void AddCountryGradeTopic_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            String sql = "INSERT INTO user_env_topic_relation (userType,env,tid) VALUES (@userType,@env,@tid)";
